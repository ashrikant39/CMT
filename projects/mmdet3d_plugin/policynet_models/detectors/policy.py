...
def generate_range_image(points, H=64, W=2048, min_depth=0.0, max_depth=190.0, scan_unfolding=False):
...
def find_2d_bbox_from_3d_corners(box_corners, H=64, W=2048):
    """
    ...
    for i in range(box_corners.shape[0]):  # Iterate over each bounding box
        ...
        min_row, max_row =np.clip(row_indices.min() , 0, 63),  np.clip(row_indices.max(), 0, 63)
        min_col, max_col = np.clip(col_indices.min(), 0, 2047),  np.clip(col_indices.max() , 0, 2047)
...
def generate_binary_mask(all_corners, H=64, W=2048):
    """
...
class PolicyDetector(MVXTwoStageDetector):
    ...
    def ste_point_selection_first(self, mask_logits, range_image, lidar_points, selected_class=1, tau=1.0):
        """
     Use a straight-through estimator to select points for voxelization (handles batch size).

        Args:
            mask_logits (Tensor): Logits for the mask of shape [B, num_classes, H, W].
            range_image (Tensor): Tensor of shape [B, N, 3] where:
                          - range_image[b][:, 0] contains row indices,
                          - range_image[b][:, 1] contains col indices.
            lidar_points (list[Tensor]): A list of length B. Each element is a tensor of shape [N, 5]
                                 with columns (x, y, z, row_idx, col_idx).
            selected_class (int): The channel index for selection.
            tau (float): Temperature for Gumbel Softmax.

        Returns:
        list[Tensor]: A list of length B, where each element is a tensor of selected (x, y, z) points.
        """
        B = mask_logits.shape[0]
        selected_points_list = []

        for b in range(B):
            # Compute the soft and hard masks for the b-th batch element.
            # Here we apply Gumbel softmax along the channel dimension (dim=0) of the individual sample.
            soft_mask = F.gumbel_softmax(mask_logits[b], tau=tau, hard=False, dim=0)   # shape: [num_classes, H, W]
            hard_mask = F.gumbel_softmax(mask_logits[b], tau=tau, hard=True, dim=0)     # shape: [num_classes, H, W]
            # Use the straight-through estimator trick:
            mask_ST = hard_mask + soft_mask - soft_mask.detach()  # In forward: equals hard_mask

            # For the current batch element, range_image[b] is [N, 3]:
            # Extract row and column indices (make sure they are long integers).
            rows = range_image[b][0,:, 0].long()  # shape: [N]
            cols = range_image[b][0,:, 1].long()  # shape: [N]

            # Use the selected channel from mask_ST to get per-pixel selection probabilities.
            selection_prob = mask_ST[selected_class, rows, cols]  # shape: [N]

            thresh = torch.quantile(selection_prob, 0.6) # selection of 65% of the data

            # Hard selection in the forward pass
            #selection = (selection_prob >= 0.5)  # boolean mask of shape [N]
            sparse_weight = torch.sigmoid(50.0 * (selection_prob - thresh))  # higher than threshold
            # For the current batch, lidar_points[b] is [N, 5].
            pts = lidar_points[b]  # shape: [N, 5]
            selected_pts = pts[:, :] * sparse_weight.unsqueeze(1)
            # Append the selected points to the list
            selected_points_list.append(selected_pts)

        return selected_points_list

    def ste_point_selection_nondiff(self, mask_logits, range_image, lidar_points, selected_class=1, tau=1.0):
        """
        Use a straight-through estimator to select points for voxelization (handles batch size).
        ...
        for b in range(B):
            ...
            # Use the selected channel from mask_ST to get per-pixel selection probabilities.
            selection_prob = mask_ST[selected_class, rows, cols]  # shape: [N]
            # Threshold to create a binary selection.
            #thresh = torch.quantile(selection_prob, 0.6) # selection of 65% of the data
            #selection = (selection_prob > thresh)  # boolean mask of shape [N]
            #print(selection)
            # Determine the number of points to select (exactly 35%).
            N = selection_prob.shape[0]
            k = int(0.4 * N) #in
            # Ensure at least one point is selected.
            k = max(k, 1)

            # Use torch.topk to select the indices of the top k values.
            topk = torch.topk(selection_prob, k, largest=True)
            # Create a boolean mask with exactly k True values.
            selection = torch.zeros_like(selection_prob, dtype=torch.bool)
            selection[topk.indices] = True

            # For the current batch, lidar_points[b] is [N, 5].
            pts = lidar_points[b]  
            #print(pts.shape)
            # Select only the (x, y, z) coordinates for points where selection is True.
            selected_pts = pts[selection, :].to(pts.device)
            selected_points_list.append(selected_pts)
    ...
    def forward_train(self,
        ...
        for gt_box in gt_bboxes_3d:
            ...
            if corners.shape[0] == 0:
                ...
                mask_np = np.zeros((64, 2048), dtype=np.uint8)
            else:
                # Compute the 2D bounding boxes from 3D corners.
                all_corners = find_2d_bbox_from_3d_corners(corners, H=64, W=2048)
                # Generate the binary mask from the 2D bounding boxes.
                mask_np = generate_binary_mask(all_corners, H=64, W=2048)
        ...
        lambda_sparse = 1
        #loss_mask = lambda_sparse * F.l1_loss(mask, torch.zeros_like(mask))#L1 gives only 0.5 
        #loss_mask = lambda_sparse * torch.mean(mask[:, 1, :, :]) #instead of sparsity guding to generate this
        #loss_mask = lambda_sparse * F.cross_entropy(mask, guidance_masks)
        loss_mask =10 * focal_loss(mask, guidance_masks, alpha=[0.2, 0.8], gamma=2.0)
        #freq = [0.7, 0.3]  # For example: 70% zeros, 30% ones.
        #inv_freq = [1/f for f in freq]
        #total = sum(inv_freq)
        #class_weights = torch.tensor([w / total * 2 for w in inv_freq], dtype=torch.float, device=mask.device)

        #loss_mask = lambda_sparse * F.cross_entropy(mask, guidance_masks, weight=class_weights)
        #filled_mask=differentiable_fill_class1_mask(mask, kernel_size=7, threshold_ratio=0.1)
        current_tau = self.pts_bbox_head.mask_generator.mask_generator.temperature
        #diff_filled_mask = batch_smart_geodesic_mask(mask,alpha=0.1,lam=0.5,min_area=200,sigma_color=0.05,sigma_spatial=0.5)
        ...
        points=self.ste_point_selection(filled_mask,range_image, points[0],tau=current_tau)
    ...
    def simple_test(self, queries, pred_results, range_image, points, img_metas, img=None, frame_index=None, rescale=False):
        ...
        for key in pred_results.keys():
            pred_results[key] = [pred_results[key][:, i] for i in range(num_frames)]
        mask, temp=self.pts_bbox_head.mask_generator(queries, img_metas, pred_results)
        # convert the mask to fill the hole version, appyling the fill-hole differentiable
        #diff_filled_mask = fill_class1_mask_same_shape(mask, kernel_size=35, threshold_ratio=0.1)
        #diff_filled_mask = batch_smart_geodesic_mask(mask,alpha=0.1,lam=0.5,min_area=200,sigma_color=0.05,sigma_spatial=0.5)
        #diff_filled_mask=batch_hysteresis_mask(mask, low_frac=0.1)
        #filled_mask= fill_class1_mask_prob(mask, kernel_size=45)
        #filled_mask= smart_geodesic_mask_soft_batch(mask,
        #                           alpha=0.1,
        #                           lam=0.5,
        #                           sigma_color=0.2,
        #                           sigma_spatial=0.5)
        
        #filled_mask=batch_smart_geodesic_mask(mask,
        #                           alpha=0.2,
        #                           lam=0.5,
        #                           min_area=200,
        #                           sigma_color=0.2,
        #                           sigma_spatial=0.5)
        #filled_mask = batch_smart_geodesic_mask(mask,alpha=0.1,lam=0.5,min_area=200,sigma_color=0.05,sigma_spatial=0.5)
        #diff_filled_mask= batch_soft_bernoulli_mask(
        #    filled_mask,
        #    offset=0,
        #    quantize_levels=[0.03125,0.0625,0.125,0.25,0.5,1.0]
        #)
        #diff_filled_mask = fill_class1_mask_same_shape(mask, kernel_size=7, threshold_ratio=0.1)
        selected_class = 1
        ...
        #filled_mask=fill_class1_mask_prob(filled_mask, kernel_size=7)
        #soft_mask= batch_soft_bernoulli_mask(
        #    filled_mask,
        #    offset=0,
        #    quantize_levels=[0.03125,0.0625,0.125,0.25,0.5,1.0]
        #)
        #soft_mask = batch_hysteresis_mask(mask, low_frac=0.1)
        filled_mask = batch_hysteresis_mask(mask, low_frac=0.1)
        #filled_mask = fill_class1_mask_same_shape(filled_mask, kernel_size=1, threshold_ratio=0.1)
        #filled_mask = fill_class1_mask_same_shape(filled_mask, kernel_size=1, threshold_ratio=0.1)
        soft_mask= batch_soft_bernoulli_mask(
                filled_mask,
                offset=0,
        ...
        binary_mask = (mask[:, selected_class, :, :] >= 0.5).float()
        # Compute average fraction of "selected" pixels over the entire mask.
        fraction_selected = binary_mask.mean()  # a scalar in [0,1]
        #print(mask.shape)
        current_tau = self.pts_bbox_head.mask_generator.mask_generator.temperature
        #print(points)
        ...
        points=self.ste_point_selection(soft_mask,range_image, points,tau=current_tau)
        ...
        return bbox_list , fraction_selected.item(), outs_dec, soft_mask
