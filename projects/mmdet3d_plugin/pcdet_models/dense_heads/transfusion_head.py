...
from projects.mmdet3d_plugin.pcdet_utils import loss_utils
...
@HEADS.register_module()
class TransFusionHead(BaseModule):
    """
    ...
    def __init__(
        ...
        self.dataset_name = target_assigner_config.get('dataset', 'nuScenes')
        ...
        loss_cls:dict = loss_config.loss_cls
        self.use_sigmoid_cls = loss_cls.get("use_sigmoid", False)
        if not self.use_sigmoid_cls:
            self.num_classes += 1
        self.loss_cls = loss_utils.SigmoidFocalClassificationLoss(gamma=loss_cls.gamma,alpha=loss_cls.alpha)
        self.loss_cls_weight = loss_config.loss_weights.cls_weight
        self.loss_bbox = loss_utils.L1Loss()
        self.loss_bbox_weight = loss_config.loss_weights.bbox_weight
        self.loss_heatmap = loss_utils.GaussianFocalLoss()
        self.loss_heatmap_weight = loss_config.loss_weights.hm_weight
        self.loss_iou_rescore_weight = loss_config.loss_weights.loss_iou_rescore_weight
        ...
        # Prediction Head
        heads = copy.deepcopy(separate_head_cfg.head_dict)
        heads['heatmap'] = dict(out_channels=self.num_classes, num_conv=num_hm_conv)
        self.prediction_head = SeparateHead_Transfusion(hidden_channel, 64, 1, heads, use_bias=True)

        self.init_weights()
        self.bbox_assigner = HungarianAssigner3D(**target_assigner_config.hungarian_assigner)
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, preds_dict):
        ...
        for idx in range(len(gt_bboxes_3d)):
            ...
            if width > 0 and length > 0:
                radius = centernet_utils.gaussian_radius(length.view(-1), width.view(-1), self.target_assigner_config.gaussian_overlap)[0]
                radius = max(self.target_assigner_config.min_radius, int(radius))
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, pred_dicts, **kwargs):
        ...
        preds = torch.cat([pred_dicts[head_name] for head_name in self.separate_head_cfg.head_order], dim=1).permute(0, 2, 1)
        code_weights = self.loss_config.loss_weights['code_weights']
    ...
    def decode_bbox(self, heatmap, rot, dim, center, height, vel, filter=False):
        ...
        """
        score_thresh = self.post_processing.score_thresh
        post_center_range = self.post_processing.post_center_range
    ...
    def get_bboxes(self, preds_dicts):
        ...
        for i in range(batch_size):
            ...
            # IOU refine 
            if self.post_processing.get('use_iou_to_rectify_score', False) and batch_iou is not None:
                pred_iou = torch.clamp(batch_iou[i][0][cmask], min=0, max=1.0)
                IOU_RECTIFIER = scores.new_tensor(self.post_processing.iou_rectifier)
            ...
            for task in self.tasks:
                ...
                if task["radius"] > 0:
                    ...
                    task_nms_config = copy.deepcopy(self.post_processing.nms_config)
                    ...
                    task_keep_indices, _ = model_nms_utils.class_agnostic_nms(
                            ...
                            nms_config=task_nms_config, score_thresh=task_nms_config.score_thresh)
                else:
