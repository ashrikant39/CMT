...
test_pipeline = [
    ...
    dict(type='NormalizeMultiviewImage', **img_norm_cfg),
    dict(type='PadMultiViewImage', size_divisor=32),                                        # Pad images to a factor of 32 size for downsampling 
    dict(
        type='DefaultFormatBundle3D',                                                       # Combine 6 images to one tensor
        class_names=class_names,
        with_label=False),                                                          
    dict(type='Collect3D', keys=['points', 'img'])                                          # Collect useful keys for train/val
]
